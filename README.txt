This package is an automator application for Mac systems
...
1.5 If this is your first time installing python, upon completion open a terminal window and install the following packages: pandas, netcdf4 via the command "conda install pandas", "conda install netcdf4" (with no quotes)

2. In order to run this package you have to change the directories of the all the scripts to correspond to your system and installation of python directory.

- Edit the clean_drifter_data_local.sh file to insert the directory of your python3 file (default install location is usually /Users/USER/miniforge3/bin/python3) and the clean_drifter_data_app_local.py script 

- In the search bar in the top right, search for the Automator app and open it. Open the Clean_drifter_data_local.app file with it. Copy the directory of the clean_drifter_data_local.sh and place into the automator script.
